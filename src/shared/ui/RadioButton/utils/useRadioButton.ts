...
export function useRadioGroup<ValueType extends string = string>(props: UseRadioGroupProps<ValueType>): UseRadioGroupResult<ValueType> {
  const {
    name,
    value,
    defaultValue,
  ...
  const [valueState, setValueState] = React.useState(defaultValue ?? options[0]?.value?.toString());
